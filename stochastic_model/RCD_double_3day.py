...
"""
RCD_double_3day.py
doubling the treatment seeking rate
...
# in this version, we test the effect of doubling the number of cases 
# arriving at a health facility per day
# but first, we must calibrate to the current treatment seeking level
...
def stoc_eqs(INPUT, lop, countExceed): 
    ...
    if lop%nDays==0:
        iota = I0*treatment_seeking*2 #doubling of the proportion of cases seeking treatment
        phi = phi_consts*iota
    else:
        phi = np.zeros((n))
...
#Save outputs
with open('../outputs/outI_RCD_double_3day.csv', 'ab') as f:
