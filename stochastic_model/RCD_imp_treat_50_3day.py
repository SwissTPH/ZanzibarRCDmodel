...
# treat 50% of cases for outbound travellers from the mainland
# and return travellers from Zanzibar
A = 0.5
B = 0.5

theta_out = theta.copy() #.copy()used because otherwise python assigns a pointer, which changes theta as well
...
MaxTime = 365*40.
I0 = np.round(pop*I_eq) #start at the equilibrium prevalence
S0 = pop-I0
...
def stoc_eqs(INPUT, lop, countExceed): 
    ...
    for k in range(n):
        for i in range(n):
            I_eff[i] = np.dot(N*Y0, np.transpose(theta_out[i,:]))/np.dot(N,np.transpose(theta[i,:]))
        
        Rate[k] = np.multiply(S0[k], np.dot(beta*I_eff, theta_in[:,k])); Change[k,:] = [-1, +1]  # transmission events
        Rate[n+k] = mu[k]*I0[k]; Change[n+k,:] = [+1, -1]  # recovery events
        Rate[2*n+k] = min(I0[k],nDays*phi[k]*I0[k]/N[k]); Change[2*n+k,:] = [+1, -1]  # RCD events
    Rate[3*n-1] = 0 # ensure no RCD on mainland
...
#Save outputs
with open('../outputs/outI_RCD_imp_treat_50_3day.csv', 'ab') as f:
